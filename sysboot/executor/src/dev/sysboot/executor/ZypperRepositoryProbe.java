...
public final class ZypperRepositoryProbe implements InstalledProbe {

  private static final Duration PROBE_TIMEOUT = Duration.ofSeconds(15);

  private final ShellRunner shellRunner;

  public ZypperRepositoryProbe(ShellRunner shellRunner) {
    this.shellRunner = shellRunner;
  }

  @Override
  public boolean supports(ItemType itemType) {
    return itemType == ItemType.ZYPPER_REPOSITORY;
  }
    ...
    return switch (result.exitCode()) {
      ...
      default ->
          new InstallationStatus.Unknown(
              repoFilePath,
              "Zypper repository probe failed (exit %d): %s"
  ...
  public InstallationStatus probe(ModuleItem item) {
    if (item.sourceSetup().orElse(null) instanceof ZypperRepositorySourceSetup setup) {
      return configuredProbe(item, setup);
    }
    return probe(item.key());
  }

  private InstallationStatus configuredProbe(ModuleItem item, ZypperRepositorySourceSetup setup) {
    ...
    boolean configMatches =
        setup.repositoryId().equals(values.get("name"))
            ...
            && (setup.autoRefresh() ? "1" : "0").equals(values.get("autorefresh"))
            && values.size() == (setup.gpgKeyUrl().isPresent() ? 6 : 5);
